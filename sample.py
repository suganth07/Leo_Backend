...
async def match_faces(file: UploadFile = File(...), folder_id: str = Form(...)):
    try:
        ...
        uploaded_embedding = faces[0].normed_embedding
        ...
        async def event_stream():
            ...
            for i, item in enumerate(known_data):
                dist = np.dot(uploaded_embedding, item["encoding"]) / (norm(uploaded_embedding) * norm(item["encoding"]))
                if dist > 0.5: # Cosine similarity threshold
                    matched.append({
                        "id": item["id"],
                        "name": item["name"],
                        "url": f"https://drive.google.com/uc?export=download&id={item['id']}"
